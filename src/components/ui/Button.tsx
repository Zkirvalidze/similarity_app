...
import { Loader2 } from 'lucide-react';
 export const buttonVariants = cva(
  'active:scale-95 inline-flex items-center justify-center rounded-md text-sm font-medium transition-color focus-ring-2 focus:ring-slate-400 focus:ring-offset-2 disabled:opasity-50  dark:ring-slate-400 disabled:pointer-events-none dark:ring-offset-slate-900',
  {
    variants: {
      variant: {
        default:
...
const Button: FC<ButtonProps> = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, isLoading, size, children, variant, ...props }, ref) => {
    return (
      <button
